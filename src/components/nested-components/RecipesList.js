...
const Recipe = ({ title, image, index, setRecipeIndex, prepTime, calories }) => {
    return(
        <div className="recipe" onClick={() => setRecipeIndex(index)}>
            <div className="recipe-image" style={{ backgroundImage: `url('${image}')` }}>
                <div className="recipe-badge-container">
                    {prepTime > 0 && <span className="recipe-badge time"><i className="fa-regular fa-clock"></i> {prepTime}m</span>}
                    {calories > 0 && <span className="recipe-badge cal"><i className="fa-solid fa-fire"></i> {Math.round(calories)} kcal</span>}
                </div>
