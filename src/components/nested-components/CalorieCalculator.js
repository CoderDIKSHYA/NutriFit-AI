...
// RapidAPI credentials are now handled by the backend BFF proxy to prevent token leaks
...
const getCalorieCalculatorApiRequestUrl = (data) => {
    ...
    return `${backendUrl}/api/calories/calculate?age=${data.age}&gender=${data.gender}&height=${data.height}&weight=${data.weight}&activitylevel=${data.activityLevel}`
}
...
const CalorieCalculator = ({ setCalculationResult}) => {
    ...
    const handleCalculateSubmit = async (e) => {
        ...
        // Make a call to the calorie calculator api via backend proxy
        const calculationResponse = await fetch(requestUrl);
